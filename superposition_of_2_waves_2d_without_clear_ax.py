...
def reset_parameter():
    ...
    k1 = k_initial
    k2 = k_initial
    omega1 = omega_initial
    omega2 = - omega_initial
    a1 = a_max
    a2 = a_max
    # Set Initial value of spinbox
    var_a1.set(a1)
    var_k1.set(k1)
    var_o1.set(omega_initial)
    var_a2.set(a2)
    var_k2.set(k2)
    var_o2.set(- omega_initial)
...
def set_axis():
    ...
    ax1.set_ylabel('y; Amplitude')
    ax1.grid()

    ax2.set_xlim(x_min, x_max)
    ax2.set_ylim(y_min, y_max)
    ax2.set_title('Superposed wave')
    ax2.set_xlabel('x; Phase (* pi)')
    ax2.set_ylabel('y; Amplitude')
    ax2.grid()


def update(f):
    global tx_step, tx_ph_v1, tx_ph_v2, tx_grp_v, y1, y2, sin_curve1, sin_curve2, y_superposed, superposed_curve

    tx_step.set_text(" Step(as t)=" + str(f))
    if k1 != 0:
        vp1 = omega1 / k1
        tx_ph_v1.set_text(" Phase velocity1(omega1/k1)=" + str(vp1))
    if k2 != 0:
        vp2 = omega2 / k2
        tx_ph_v2.set_text(" Phase velocity2(omega2/k2)=" + str(vp2))
    if (k1 - k2) != 0:
        vg = (omega1 - omega2) / (k1 - k2)
        tx_grp_v.set_text(" Group velocity(d_omega/dk)=(omega1-omega2)/(k1-k2)=" + str(vg))
    else:
        tx_grp_v.set_text(" None group velocity")

    # Draw sin curve
    y1 = a1 * np.sin((k1 * x) * np.pi - omega1 * f)  # sin(kx - omega*t)
    y2 = a2 * np.sin((k2 * x) * np.pi - omega2 * f)  # sin(kx - omega*t)
    y_superposed = y1 + y2
    sin_curve1.set_ydata(y1)
    sin_curve2.set_ydata(y2)
    superposed_curve.set_ydata(y_superposed)
...
y1 = a1 * np.sin((k1 * x) * np.pi - omega1 * 0)  # sin(kx - omega*t)
y2 = a2 * np.sin((k2 * x) * np.pi - omega2 * 0)  # sin(kx - omega*t)
y_superposed = y1 + y2
...
tx_step = ax1.text(x_min, y_max * 0.9, " Step(as t)=" + str(0))
tx_ph_v1 = ax1.text(x_min, y_max * 0.7, " Phase velocity1(omega1/k1)=" + str(0))
tx_ph_v2 = ax1.text(x_min, y_max * 0.5, " Phase velocity2(omega2/k2)=" + str(0))
tx_grp_v = ax2.text(x_min, y_max * 0.9, " Group velocity(d_omega/dk)=(omega1-omega2)/(k1-k2)=" + str(0))
sin_curve1, = ax1.plot(x, y1, linestyle='-', label="A1=" + str(a1) + ", k1=" + str(k1) + ", omega1=" + str(omega1))
sin_curve2, = ax1.plot(x, y2, linestyle='-', label="A2=" + str(a2) + ", k2=" + str(k2) + ", omega2=" + str(omega2))
superposed_curve, = ax2.plot(x, y_superposed, linestyle='-')
...
var_k1.set(k_initial)  # Initial value
...
var_o1.set(omega_initial)  # Initial value
...
var_k2.set(k_initial)  # Initial value
...
var_o2.set(- omega_initial)  # Initial value
